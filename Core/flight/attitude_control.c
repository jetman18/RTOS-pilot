...
static float roll_trim = 0,pitch_trim = 0;
...
void attitude_ctrl_init(){
   gps_lost = 1;
   // init pid 
   pid_init(&roll_rate,3,0,0.5,0,100,LOOP_TIM);
   pid_init(&roll_angle,10,0,0,100,100,LOOP_TIM);

   pid_init(&pitch_rate,3,0,0,100,100,LOOP_TIM);
   pid_init(&pitch_angle,10,0,0,100,100,LOOP_TIM);
}
...
void attitude_ctrl(){ 
   ...
   // float yaw_r = AHRS.yaw_rate;
    ...
    //float yaw = AHRS.yaw;
    roll_cmd = ((int)ibusChannelData[0] - 1500)*0.1f;
	pitch_cmd = ((int)ibusChannelData[1] - 1500)*-0.1f;
    ...
    if(ibusChannelData[CH5] > 1600 ){
        ...
        const float roll_pid_gain = ((int)ibusChannelData[CH7] - 1000)*0.001f;
